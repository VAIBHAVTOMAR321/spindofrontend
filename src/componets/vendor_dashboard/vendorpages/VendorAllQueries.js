...
const VendorAllQueries = () => {
  ...
  const fetchQueries = () => {
    ...
    setLoading(true);
    const apiUrl = `https://mahadevaaya.com/spindo/spindobackend/api/vendor/request/?unique_id=${user.uniqueId}`;
    fetch(apiUrl, {
      ...
      .then((data) => {
        if (data.status && Array.isArray(data.data)) {
          ...
          setError("Failed to load vendor queries.");
        }
        setLoading(false);
      })
      .catch(() => {
        setError("Error fetching vendor queries.");
        setLoading(false);
      });
  };
                  ...
                  {!loading && !error && (
                    <>
                      ...
                      <Table responsive bordered hover className="rounded-4 shadow-sm">
                        <thead className="table-thead-vendor" style={{ background: "linear-gradient(90deg, #2b6777 0%, #52ab98 100%)", color: "#fff" }}>
                          <tr>
                            <th>#</th>
                            <th>Title</th>
                            <th>Issue</th>
                              ...
                              <img
                                src={
                                  selectedQuery.issue_image.startsWith("http")
                                    ? selectedQuery.issue_image
                                    : `https://mahadevaaya.com/spindo/spindobackend/media/vendor_query_images/${selectedQuery.issue_image}`
                                }
                                alt="Query"
...
export default VendorAllQueries;
